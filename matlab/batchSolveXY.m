...
elseif option == 2 
    ...
    % case of Binv Yinv = Xinv Ainv
    % n_Binv*f_Binv*delta_Yinv = delta_Xinv*n_Ainv*f_Ainv
    % Note that in this case, Binv ~ A, Yinv ~ X, Xinv ~ Y , Ainv ~ A
    % The same goes for the calculated means and Sigs
    SigA = SigA - SE3inv_Ad(MeanA)*nstd_A*eye(6,6)*SE3inv_Ad(MeanA)';
    SigB = SigB - SE3inv_Ad(MeanB)*nstd_B*eye(6,6)*SE3inv_Ad(MeanB)';
%     SigA = nstd_A;
%     SigB = nstd_B;
end
